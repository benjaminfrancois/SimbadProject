...
class SimpleImage extends Frame {
	...
	public SimpleImage(String __filename, boolean __display) {
		...
		try { // (1) wait for image to be loaded (2) timeout in 10s
			if (!mediaTracker.waitForID(1, 10000)) {
				...
				System.err.println(
						"  " + System.getProperty("user.dir") + System.getProperty("file.separator") + _filename);
				System.exit(-1);
			}
	...
	public void setPixel(int __x, int __y, int __alpha, int __red, int __green, int __blue) {
		...
		_pixelMatrix[__y * _width + __x] = __alpha * (int) Math.pow(256, 3) + __red * (int) Math.pow(256, 2)
				+ __green * 256 + __blue;
	}
	...
	public void exampleImageManipulation_1() {
		/*
		 * for( int i = 0; i < (_width*_height) ; i++ ) _pixelMatrix[i] =
		 * _pixelMatrix[i] & 0xFFFFFF; // alpha, R, G, B -- alpha=255=maxOpacity //
		 * ...SAME AS...
		 **/
	...
	public void exampleImageManipulation_2() {
		for (int x = 0; x != _width; x++)
			for (int y = 0; y != _height; y++) {
				int[] values = getPixel(x, y);
				setPixel(x, y, 255, (values[1] + 1) % 256, (values[2] + 1) % 256, (values[3] + 1) % 256); // alpha, R,
																											// G, B --
																											// alpha=255=maxOpacity
			}
	}
	...
	public void exampleImageManipulation_3() {
		for (int x = 10; x != _width - 10; x++)
			for (int y = 10; y != _height - 10; y++) {
				int[] values = getPixel((int) (x - 8 * Math.sin(x)), (int) (y - 4 * Math.cos(y)));
				setPixel(x, y, 255, (values[1] + 1) % 256, (values[2] + 1) % 256, (values[3] + 1) % 256); // alpha, R,
																											// G, B --
																											// alpha=255=maxOpacity
			}
	...
	public static void main(String[] args) {
		// Display Examples
		/*
		 * boolean display = false; SimpleImage simpleimage = new
		 * SimpleImage("nicolas/test.gif", display); simpleimage.displayInformation();
		 * while (true) { simpleimage.exampleImageManipulation_3(); } //if ( display )
		 * obj.repaint();
		 */
		...
		for (int y = 0; y != simpleimage.getHeight(); y++) {
			for (int x = 0; x != simpleimage.getWidth(); x++) {
				...
				if (values[1] == 255 && values[2] == 0 && values[3] == 0)
					System.out.print("#");
				else if (values[1] == 0 && values[2] == 255 && values[3] == 0)
					System.out.print("X");
				else if (values[1] == 0 && values[2] == 0 && values[3] == 255)
					System.out.print("!");
				else
					System.out.print(" ");
			}
