...
public class Menu {
	...
	private void initialize() {
		...
		JLabel lblScorescore = new JLabel(
				"Score : " + ((simbad.getChara() != null) ? simbad.getChara().getScore() : "0"));
		...
		JButton btnNewButton = new JButton("Up  speed ( - "
				+ Math.round((simbad.getChara().getSpeed() + 2) * (SCORE_UP * simbad.getChara().getSpeed())) + " )");
		btnNewButton.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent e) {
				int count = (int) Math
						.round((simbad.getChara().getSpeed() + 2) * (SCORE_UP * simbad.getChara().getSpeed()));
		...
		JButton btnUpPower = new JButton("Up  power ( - "
				+ Math.round((simbad.getChara().getPower() + 2) * (SCORE_UP * simbad.getChara().getPower())) + " )");
		btnUpPower.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent arg0) {
				int count = (int) Math
						.round((simbad.getChara().getPower() + 2) * (SCORE_UP * simbad.getChara().getPower()));
				...
				if (simbad.getChara().getScore() >= count) {
					...
					lblScorescore
							.setText("Score : " + ((simbad.getChara() != null) ? simbad.getChara().getScore() : "0"));
