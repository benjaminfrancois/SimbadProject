...
public class Zombie extends Agent {
	...
	private final double DROP_RATE = 0.95; // 5 %
	...
	public void performBehavior() {
		...
		if (!isDead) {
			...
			if (getVeryNearAgent() != null) {
				...
				if (getVeryNearAgent() instanceof Character) {
			...
			if (test) {
				...
				if (cibleX - 0.3 > myPosX) {
					if (cibleZ - 0.3 > myPosZ)
						translateTo(new Vector3d(+BASE_SPEED * speed, 0, +BASE_SPEED * speed));
					else if (cibleZ + 0.3 < myPosZ)
						translateTo(new Vector3d(+BASE_SPEED * speed, 0, -BASE_SPEED * speed));
					else
						translateTo(new Vector3d(+BASE_SPEED * speed, 0, 0));
					...
					if (cibleZ - 0.3 > myPosZ)
						translateTo(new Vector3d(0, 0, +BASE_SPEED * speed));
					else if (cibleZ + 0.3 < myPosZ)
						translateTo(new Vector3d(0, 0, -BASE_SPEED * speed));
					else
