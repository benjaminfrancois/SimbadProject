...
public class PhysicalEngine {
	...
	/**
	 * Check all agents/static objetc pairs and verify physical interactions and/or
	 * collision.
	 */
	...
	protected boolean intersect(BoundingSphere bs, BoundingBox bb) {
		 ...
		 * bb.getUpper(p2); bs.getCenter(p3); double xmin = Math.min(p1.x,p2.x); double
		 * ymin = Math.min(p1.y,p2.y); double zmin = Math.min(p1.z,p2.z); double xmax =
		 * Math.max(p1.x,p2.x); double ymax = Math.max(p1.y,p2.y); double zmax =
		 * Math.max(p1.z,p2.z);
		 * 
		 * double dmin = 0; if (p3.x < xmin) dmin += (p3.x - xmin)*(p3.x - xmin); else
		 * if (p3.x >xmax ) dmin += (p3.x - xmax)*(p3.x - xmax);
		 * 
	...
	protected void computeContactNormal(BoundingSphere bs, BoundingBox bb, Vector3d n) {
		...
		/*
		 * p = projNormal(p2.x,p1.y,p2.z, p2.x,p2.y,p2.z, p1.x,p1.y,p2.z); if (p < min)
		 * { min = p; n.set(v3);}
		 */
		/*
		 * p = projNormal(p1.x,p1.y,p2.z, p1.x,p2.y,p2.z, p1.x,p1.y,p1.z); if (p < min)
		 * { min = p; n.set(v3);}
		 */
		/*
		 * p = projNormal(p1.x,p2.y,p1.z, p1.x,p2.y,p2.z, p2.x,p2.y,p1.z); if (p < min)
		 * { min = p; n.set(v3);} p = projNormal(p1.x,p1.y,p2.z, p1.x,p1.y,p1.z,
		 * p2.x,p1.y,p2.z); if (p < min) { min = p; n.set(v3);}
		 */
