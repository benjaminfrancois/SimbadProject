...
public class SimpleAgent extends BaseObject {
	 ...
	 * 
	 * @param pickableSceneBranch The scene branch containing all collidable
	 *                            objects.
	 */
				...
				for (int i = 0; i < picked.length; i++) {
					...
					if (obj instanceof Shape3D) {
						if (((Shape3D) obj).intersect(picked[i], collisionPickBounds, collisionDistance)) {
							// System.out.println("dist " + collisionDistance[0] + " tol
							// "+collisionTolerance);
	...
	/**
	 * Returns printable description of the agent. This may be multiline and complex
	 * in subclasses.
	 * 
	...
	/**
	 * Returns the actuator device designated by num. User will have to cast to the
	 * appropriate class.
	 * 
	...
	/**
	 * called back by simulator when a physical interaction as occured with an other
	 * agent.
	 */
