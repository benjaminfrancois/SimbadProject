...
public class RangeSensorBelt extends PickSensor {
	 ...
	 *                  TYPE_BUMPER.
	 * @param nbsensors - the number of sensors in the belt (typically 4,6,12,24 or
	 *                  36).
	 ...
	 *                   sensor positions. the magnitude of the vector corresponds
	 *                   to the max range.
	 */
				...
				if (picked != null) {
					...
					for (int i = 0; i < picked.length; i++) {
						...
						if (obj instanceof Shape3D) {
							if (((Shape3D) obj).intersect(picked[i], pickSegment, dist)) {
								if (dist[0] < minDist) {
									minDist = dist[0];
									intersect = true;
								}
	...
	/**
	 * Returns the last measure collected for the individual sensor. Measurement is
	 * made from the circle perimeter.
	 * 
